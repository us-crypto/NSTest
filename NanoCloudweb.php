...
$user_ip = getUserIP();
$norm_ip = getVisIPAddr();
if (isset($_SESSION['loggedin'])&&$_SESSION['loggedin']==true) {
    echo '
    welcome back '.$_SESSION['name'].', logged in via 
    "' . $_SESSION['mail'] . '" u nasty bitch :D   well then suck it up 8=====>';
    if ($_SESSION['loggedin']) {
        $sql = 'INSERT INTO `logs`( `IP`, `NAME`, `TIME`, `NORMIP`) 
        VALUES ("'.$user_ip.'","'.$_SESSION['name'].'","'.$_SESSION['time'].'","'.$norm_ip.'")';
    ...
    if (isset($_SESSION['warning'])&&$_SESSION['warning']) {
        $sql = 'INSERT INTO `attacker_cloud`(`IP`, `IPNOVPN`) 
        VALUES ("'.$user_ip.'","'.$norm_ip.'")';
        $result = $conn->query(($sql));
    }
...
function admin($conn){
    $sql = 'SELECT `USERID`, `NAME`, `EMAIL`, `PASSWORD` FROM `user_tb`';
    $result = $conn->query($sql);
    $html = '
        <table>
            <thead>
            ...
            <tbody>
        ';
    foreach ($result as $dvv) {
        $NAME=$dvv['NAME'];
        $EMAIL=$dvv['EMAIL'];
        $PASSWORD =$dvv['PASSWORD'];
        $USERID =$dvv['USERID'];
        $html .= '
            <tr>
                <td><input type="checkbox" name="choice[]" value="'.$USERID .'">'.$USERID.'</td>
                <td>'.$NAME.'</td>
                <td>'.$EMAIL.'</td>
                <td>'.$PASSWORD.'</td>
            </tr>
    ...
    <form method="post" id="frm" enctype="multipart/form-data">
        ...
        <input type="hidden" name="wasTun" id="wasTun">
            ...
            <main>
                ...
                <?php
                echo '  <div class="h3text">
                <h3>available users</h3>
                </div>';

                if ($_SESSION['user']=='ceo@nano-suits.com') {
                    $admin = admin($conn);
                    echo $admin;
                }
                ?>
